...
function tryDevice( device ) {
	...
	getResources( device )
		.then( function( resourceList ) {
			...
			desiredNewResource.endpoint = pickEndpoint( resourceList.target.endpoints );
			return client.create( desiredNewResource, resourceList.target );
		} )
		.then( function( newResource ) {
			delete desiredNewResource.endpoint;
			console.log( JSON.stringify( { assertion: "deepEqual", arguments: [
				filterKeys( newResource, [
					"resourcePath", "interfaces", "resourceTypes", "discoverable", "secure"
				] ),
				desiredNewResource,
